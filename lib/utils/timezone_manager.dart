...
class TimeZoneManager {
  ...
  static SimpleTime getTimeForTimeZone(String timeZone) {
    if (timeZone == 'UTC' || timeZone.trim() == '') {
      ...
      if (timeZoneLabel == null) {
        ...
        log("utc time: ${SimpleTime(currentTime.toUtc().hour, currentTime.toUtc().minute)}");
        return convertTo12HourStanderd(timeToReturn);
      } else {
        timeToReturn = timeToReturn + getTimeToAdd(timeZoneLabel);
        log("time returned: $timeToReturn");
        log("utc time: ${SimpleTime(currentTime.toUtc().hour, currentTime.toUtc().minute)}");
        return convertTo12HourStanderd(timeToReturn);
      }
  ...
  static SimpleTime getTimeToAdd(String data) {
    ...
    if (timeToAddString == '') {
      ...
      timeToReturn.hour = timeToReturn.hour;
      timeToReturn.minute = timeToReturn.minute;
  ...
  static String? findLabelFromTimeZonesList(String data) {
    Map<String, String>? timeZoneLabel;

    try {
      timeZoneLabel =
          timeZones.firstWhere((element) => element['value'] == data);
    } catch (e) {
      log(e.toString());
    }
...
class SimpleTime {
  ...
  SimpleTime operator +(SimpleTime otherTime) {
    ...
    // SimpleTime timeToReturn =
    //     SimpleTime(this.hour + otherTime.hour, this.minute + otherTime.minute);
    // if (timeToReturn.minute >= 60) {
    //   timeToReturn.hour += 1;
  ...
  String toString() {
    ...
    String minuteInString = minute.toString();
    if (hour < 10 && hour > 0) {
      hourInString = '0$hour';
    }
    if (minute < 10) {
